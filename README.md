...
- 역 클릭시 역의 현재 날씨, 습도, 보관함 정보 표시
    - 날씨는 [Open weather API](https://openweathermap.org/api) 사용하여 표시
    -  사물함마다 상태(대여중, 대여가능, 점검 중)를 다른 색으로 표시
- 회원가입 / 로그인 / 로그아웃 기능
- 게시판에 메세지 게시 / 게시글에 답글 기능
- 마이 페이지 기능
    - 로그인한 유저가 빌린 사물함, 게시한 게시물과 해당하는 댓글 확인이 가능한 페이지
- 관리자 페이지 기능
    - 게시물, 댓글, 역, 락커, 유저를 삭제, 복원, 수정이 가능한 페이지
